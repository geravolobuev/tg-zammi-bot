...
async function runOpenRouterFullOcr(apiKey, imageDataUrl, preferredModel) {
  const modelCandidates = [preferredModel, 'qwen2.5-vl-72b-instruct:free', 'google/gemma-3-27b-it:free', 'mistralai/mistral-small-3.1-24b-instruct:free', 'google/gemma-3-12b-it:free', 'nvidia/nemotron-nano-12b-v2-vl:free', 'openrouter/free'].filter(Boolean);
  ...
  for (const model of [...new Set(modelCandidates)]) {
    const response = await fetch(`${OPENROUTER_API_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        messages: [{ role: 'user', content: [{ type: 'text', text: 'Извлеки ВЕСЬ текст со страницы книги максимально точно. Верни только чистый текст без комментариев, без markdown, без JSON.' }, { type: 'image_url', image_url: { url: imageDataUrl } }] }],
        temperature: 0
      })
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error(error);
    if (message.includes('OPENROUTER_AUTH_ERROR')) return res.status(200).json({ ok: false, error: 'OPENROUTER_AUTH_ERROR' });
    if (message.includes('OPENROUTER_QUOTA_ERROR')) return res.status(200).json({ ok: false, error: 'OPENROUTER_QUOTA_ERROR' });
